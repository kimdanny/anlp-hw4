...
def save_delta_histogram(delta_df, save_dir, lamp_num):
    # Plotting the histogram of the 'delta' column
    plt.hist(delta_df["delta"], bins=40, color="blue", edgecolor="black", alpha=0.7)
    plt.title("Histogram of Delta")
    plt.xlabel("Delta")
    plt.ylabel("Frequency")
    plt.savefig(os.path.join(save_dir, f"{lamp_num}_delta_hist.png"))
...
def main(args, delta_histogram=True, delta_stats=True):
    LAMP_NUM = args.lamp_num
    MODEL_NAME = args.model_name
    EVAL_RESULTS_DIR = os.path.join(
        CUR_DIR_PATH, "eval_results", MODEL_NAME, "lamp1000"
    )
    DELTA_FP = os.path.join(EVAL_RESULTS_DIR, f"{LAMP_NUM}_delta.tsv")
    DELTA_DF = pd.read_csv(DELTA_FP, sep="\t", dtype={"qid": str, "pid": str})

    # 1. Save delta histograms in png
    if delta_histogram:
        save_delta_histogram(
            delta_df=DELTA_DF, save_dir=EVAL_RESULTS_DIR, lamp_num=LAMP_NUM
        )

    # 2. Save delta stats in tsv
    if delta_stats:
        save_delta_stats(
            delta_df=DELTA_DF, save_dir=EVAL_RESULTS_DIR, lamp_num=LAMP_NUM
        )
