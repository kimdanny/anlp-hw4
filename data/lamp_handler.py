...
class LaMPHandler:
    ...
    def _lamp_1_aip(self, question: str, profiles: List[dict]) -> str:
        ...
        max_tok_len_per_profile: int = (
            self.calculate_max_token_len_per_profile(question, k=len(profiles))
            if self.adaptive_M
            else self.calculate_max_token_len_per_profile(question, k=self.K)
        )
        titles = ", and ".join(
            [
                self._lamp_1_ppep(
        ...
        titles = trim_sentence_by_token_len(
            titles,
            self.tokenizer,
            max_tok_len=self.TOKENIZER_MAX_LENGTH
            - get_tokenized_length(question, self.tokenizer)
            - 50,
        )
    ...
    def _lamp_2_aip(self, question: str, profiles: List[dict]) -> str:
        ...
        if aip != "":
            aip += ". "
        aip = trim_sentence_by_token_len(
            aip,
