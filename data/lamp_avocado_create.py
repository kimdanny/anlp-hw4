# Adapted from https://github.com/LaMP-Benchmark/LaMP/blob/main/data/avocado/create_avocado_dataset.py
...
parser.add_argument(
    "--avocado_files_dir",
    required=True,
    help="Address to the directory containing zip files for avocado dataset 'avocado-1.0.2/data/text'",
)
parser.add_argument(
    "--extract_addr",
    required=True,
    help="A temp dir to extract the files for creating dataset",
)
parser.add_argument(
    "--output_dir", required=True, help="The directory to generate the final dataset"
)
...
parser.add_argument(
    ...
    required=True,
    help="The address to the dev_questions.json file",
)
...
parser.add_argument(
    ...
    action="store_true",
    help="Stores extra information about user and date",
)
...
if __name__ == "__main__":
    ...
    for sample in input_questions_dev:
        id = sample["input"]
        sample["input"] = (
            f"Generate a subject for the following email: {database[id]['content']}"
        )
        sample["output"] = database[id]["subject"]
        for p in sample["profile"]:
            pid = p["text"]
